...
struct ldm_addr_ip {
	...
	unsigned int port; /* (struct sockaddr_in).sin_port */
	unsigned int addr; /*(struct sockaddr_in).(struct in_addr)sin_addr.s_addr*/
};
...
% */
#endif
typedef unsigned int max_hereis_t;
...
} = 300029; /* LDM = 300029, use 0x2ffffffe for experiments */
