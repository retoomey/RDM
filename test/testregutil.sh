...
$BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" -c

echo "=== Phase II: Populating Registry Parameters ==="
$BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" -s "foo value" /foo_key
$BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" -b TRUE /fooBool_key
$BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" -u 42 /fooInt_key
$BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" -s "string value 2" /subkey/string_key
$BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" -s "to_be_deleted" /temp_key

echo "=== Phase III: Deleting a Parameter ==="
$BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" -r /temp_key
...
$BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" | sort > "$ACTUAL_OUTPUT"
...
if $BIN_DIR/$BIN_REGUTIL -d "$REG_DIR" -q /does_not_exist; then
...
if [ "$PASSED" = true ]; then
    echo "======================================================="
    echo " 🎉 ALL $BIN_REGUTIL TESTS PASSED! 🎉"
    echo "======================================================="
    rm -rf "$TEST_DIR"
    exit 0
else
    echo "======================================================="
    echo " 🚨 $BIN_REGUTIL TEST FAILED 🚨"
    echo "======================================================="
    exit 1
fi
