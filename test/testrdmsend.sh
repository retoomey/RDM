...
echo "=== 🚀 Setting up ldmsend Integration Test ==="
...
"$BIN_DIR/pqcreate" -s 5M -q "$SERVER_QUEUE"
...
"$BIN_DIR/ldmd" -P $LDM_PORT -q "$SERVER_QUEUE" -l "$SERVER_LOG" "$SERVER_CONF" &
SERVER_PID=$!
...
echo "This is a test payload sent from modern C++ ldmsend." > "$TEST_FILE"

echo "=== Phase V: Launching C++ ldmsend to Push Data ==="
"$BIN_DIR/ldmsend" -h localhost -P $LDM_PORT -f EXP "$TEST_FILE" > "$LDMSEND_LOG" 2>&1
...
"$BIN_DIR/pqcat" -q "$SERVER_QUEUE" -f EXP -p "dummy_payload" > "$PQCAT_OUT"

PASSED=true

# Check if pqcat successfully extracted the file contents
if grep -q "This is a test payload sent from modern C++ ldmsend." "$PQCAT_OUT"; then
...
if [ "$PASSED" = true ]; then
    ...
    echo "======================================================="
    echo " 🚨 LDMSEND TEST FAILED 🚨"
    echo "--- Dump of ldmsend log ---"
    cat "$LDMSEND_LOG"
    echo "======================================================="
    exit 1
fi
